...
# opds /hadoop
 ...
 - yarn管理资源，client向rm申请资源，rm分配contains运行 nm来管理contains的运行情况，am来监控应用的执行情况。
 ...
 - 压缩gzip 
 - hdfs的写是客户端通过和namenode通信，然后获取datanode信息，客户端和datanode进行通信，将数据按照128一个大小分割成block然后发送给datanode，多个datanode串行通信，依次qck确认，然后反馈给客户端，继续下一次操作
