...
- checkpoint机制
   ...
   * lsm 主要有wal（防止在mem中的数据丢失）、sstable、memtable（使用跳表来进行提升性能），写入memtable，并且进行有序存储，待超过内存限制会flush到本地的sstable，使用归并排序对数据进行有序存储，读的时候需要合并memtable和sstable的数据，sstable是有block和元数据来进行存储的，借助布隆过滤器来实现快速检索
-  背压机制
